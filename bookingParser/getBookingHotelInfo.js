...
const getBookingHotelInfo = async (multiplierArgument, link, reviewsLimit = 10) => {
  ...
  // Har bir bo'lim (asosiy info, rasmlar, sharhlar) ALOHIDA himoyalangan —
  // biri qulasa (Booking tuzilmasi o'zgargani sabab) qolganlari baribir keladi.
  // Ilgari rasm bloki qulasa, sharhlarga umuman yetib bormas edi.
  const hotelInfo = { link };
  try {
    await page.goto(link, { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(1500 * multiplier);

    //main info — `.d31796cb42` (areaInfo) bo'lmasa ham yiqilmaymiz.
    try {
      await page.waitForSelector(".d31796cb42", { timeout: 8000 * multiplier });
    } catch { /* areaInfo bo'lmasligi mumkin — davom etamiz */ }
    try {
      Object.assign(hotelInfo, await getHotelInfo(page));
      hotelInfo.link = link;
    } catch (e) { console.warn("[booking info] asosiy info xato:", e.message); }

    //place photos — ixtiyoriy, xato bo'lsa sharhlarга xalaqit bermaydi.
    try {
      ...
      hotelInfo.photos = await page.evaluate(() =>
        Array.from(document.querySelectorAll(".bh-photo-modal img")).map((el) => el.getAttribute("src")),
      );
      await page.click(".bh-photo-modal-close");
      await page.waitForTimeout(2000 * multiplier);
    } catch (e) { console.warn("[booking info] rasm bloki xato (o'tkazib yuborildi):", e.message); }

    //place reviews — alohida himoyalangan.
    try {
    ...
    } catch (e) { console.warn("[booking info] sharh bloki xato:", e.message); }
  } finally {
    await closeBrowser();
  }
